...
function isdatetime(s) {
    if (s == null || s.toString().length < 4) {  // 修改最小长度为4以支持HH:mm
        return false;
    }
    ...
    function checkDateTime(str) {
        ...
        var patterns = [
            ...
            /^\d{1,2}\/\d{1,2}\/\d{4}(\s\d{1,2}:\d{1,2}(:\d{1,2})?)?$/, // DD/MM/YYYY [HH:mm[:ss]]
            /^\d{4}年\d{1,2}月\d{1,2}日(\s\d{1,2}时\d{1,2}分(\d{1,2}秒)?)?$/, // 中文日期格式
            /^\d{1,2}月\d{1,2}日\d{4}年(\s\d{1,2}时\d{1,2}分(\d{1,2}秒)?)?$/, // 中文日期格式2
            /^\d{4}\.\d{1,2}\.\d{1,2}(\s\d{1,2}:\d{1,2}(:\d{1,2})?)?$/, // YYYY.MM.DD [HH:mm[:ss]]
            /^\d{1,2}\.\d{1,2}\.\d{4}(\s\d{1,2}:\d{1,2}(:\d{1,2})?)?$/, // DD.MM.YYYY [HH:mm[:ss]]
            /^\d{1,2}:\d{1,2}$/ // 新增: HH:mm格式
        ];
        ...
        for (var i = 0; i < patterns.length; i++) {
            if (patterns[i].test(str)) {
                ...
                // 提取日期部分
                if (i <= 1 || i === 6) {
                    // YYYY-MM-DD 或 YYYY/MM/DD 或 YYYY.MM.DD 格式
                    dateParts = str.split(/[-\/\s:\.]/);
                } else if (i === 4) {
                    // 中文日期格式 YYYY年MM月DD日
                    dateParts = str.split(/[年月日\s时分秒]/);
                } else {
                    // DD-MM-YYYY 或 DD/MM/YYYY 或 DD.MM.YYYY 格式
                    dateParts = str.split(/[-\/\s:\.]/);
                    // 调整顺序为 YYYY MM DD
