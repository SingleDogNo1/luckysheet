...
function chart(options, config, isDemo) {
    ...
    seriesLoadScripts(dependScripts, null, function() {
        ...
        chartInfo.checkCurrentBoxChange = chartmix.default.checkCurrentBoxChange;
...
function createLuckyChart(width, height, left, top) {
    ...
    console.log('jfgird_select_save :>> ', jfgird_select_save);
    if (
 ...
 * @param {Object} range 要生成图表数据的选区范围
 * @param {String} range.range 图表数据的选区范围,支持选区的格式为"A1:B2",只能为单个选区
 * @param {Number} range.order 要生成图表的工作表下标；默认值为当前工作表下标
 * @param {Number} chartType 要插入图表的类型，默认值为"echarts|line|default"
 * @param {Number} position 要插入图表的位置，默认值为当前选区的左上角单元格位置
 * @param {Number} size 要插入图表的尺寸，默认值为{width: 400, height: 250}
 * @param {Function} success 操作结束的回调函数
 */
export function insertChart({
    range,
    chartType = 'echarts|line|default',
    position = { left: 0, top: 0 },
    size={ width: 400, height: 250 },
    success
}) {
    luckysheet.setRangeShow(range.range, {
        show: false,
        order: range.order
    })
    ...
    success && success(chart_id, chart_json.chartOptions);
}

export function checkCurrentBoxChange(
chart_id, rangeRowCheck, rangeColCheck, rangeConfigCheck
) {
    console.log('chartInfo :>> ', chartInfo);
    chartInfo.checkCurrentBoxChange(chart_id, rangeRowCheck, rangeColCheck, rangeConfigCheck);
}
...
// delete chart
export function delChart(chart_id) {
    // delete container
    ...
    // delete storage
    let sheetFile = chartInfo.luckysheetfile[getSheetIndex(chartInfo.currentSheetIndex)];
    let index = sheetFile && sheetFile.chart && sheetFile.chart.findIndex((item) => item.chart_id == chart_id);
    if (index && index !== -1) {
