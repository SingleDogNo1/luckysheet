...
//cell locked state
export function checkProtectionLocked(r, c, sheetIndex, isOpenAlert=false, isLock=true){
        ...
        for(let r=r1;r<=r2;r++){
            for(let c=c1;c<=c2;c++){
                ...
                let isLock = cell.lo === undefined || cell.lo === 1, // 单元格是否锁定
                    isPass = checkProtectionLockedSqref(r, c , aut, local_protection, false, isLock);
                if(!isPass){
                    return false;
                }
            }
